        ...
        .then(function (response) {
            if (response.data.success===true) {
                ...
                response.data.data.shifts.forEach(element =>
                    {
                        $('#shift_id').append('<option value="'+element.id+'">'+element.name+'|'+convertunix(element.start_time)+'|'+element.time/60+' phút</option>');
                    }
                )
                khoitao()
...
async function deleteE() {
    ...
    axios.delete('http://localhost:5000/api/v1/shiftsRooms/?shift_room_id='+idExam,
        {
            headers: {
                'examination-token': window.localStorage.getItem('examtoken')
            }
